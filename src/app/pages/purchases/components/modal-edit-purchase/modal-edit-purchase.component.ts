...
export class ModalEditPurchaseComponent implements OnInit {
  ...
  updatePurchase() {
    if (this.purchaseForm.valid) {
      ...
      this._orderService.updatePurchase({ ...this.purchaseForm.getRawValue(), purchase_date, locker_entry_date })
        .subscribe((res: any) => {
          this.successUpload.emit(true);
          this._notify.show('Compra Actualizada', 'La orden ha sido actualizada con éxito.', 'success');
        }, err => {
          this.isLoading = false;
          this._notify.show('Error', 'No se pudo actualizar la compra, intenta de nuevo', 'error');
          throw err;
        });
    } else {
