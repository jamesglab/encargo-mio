...
import { ExportPdfService } from "src/app/pages/ecommerce/_services/export-pdf.service";
...
export class ModalShippingComponent implements OnInit {
  ...
  buildForm(shipping: any): void {
    ...
    this.updateShippingForm = this._formBuilder.group({
      ...
      conveyor: [
        this.conveyors.find((item) => item.id == shipping.conveyor),
        [Validators.required],
      ],
      ...
      shipping_type: [
        shipping.shipping_type ? shipping.shipping_type.id : null,
        [Validators.required],
      ],
      ...
      products: [
        shipping.products ? shipping.products : null,
        Validators.required,
      ],
    ...
    this.filteredConveyors =
      this.updateShippingForm.controls.conveyor.valueChanges.pipe(
        startWith(""),
        map((value) => this._filter(value, "conveyors"))
      );
    this.filteredAddress =
      this.updateShippingForm.controls.address.valueChanges.pipe(
        startWith(""),
        map((value) => this._filter(value, "address"))
      );
    this.filteredUsers =
      this.updateShippingForm.controls.user.valueChanges.pipe(
        startWith(""),
        map((value) => this._filter(value, "users"))
      );

    this.getInfoUser();
    if (this.shippingToUpdate.status == "6") {
      //ONLY FOR FRACTIONED
  ...
  async getInfoUser() {
    ...
    await this._lockers
      ...
      .subscribe(
        (locker: any) => {
          ...
          this.inLocker.map((item: any) => {
            item.button = false;
          });
          this.outLocker = locker.in_shipping;
          this.outLocker.map((item: any) => {
            item.button = false;
          });
    ...
    await this._orderService
      .validateNotProducts(this.shippingToUpdate.id)
      .subscribe(
        (res: any) => {
          this.message = { ...res };
        },
  ...
  getFractionedChildren(): void {
    //GET FRACIONED CHILDREN
    const fractionedSubscr = this.orderShippingService
      .getFractionedChildren(this.shippingToUpdate.id)
      .subscribe(
        (res) => {
  ...
  renderStatus(status: number): string {
    //USE THE CASE FUNCTION FOR RENDER STATUS OF FRACTION CHILDREN
    ...
    if (objProduct.arrived) {
      if (event.previousContainer === event.container) {
        ...
        if (type === "shipping") {
          ...
          this._dragdrop
            .moveAddProduct({
              shipping: this.shippingToUpdate.id,
              product: objProduct,
            })
            .subscribe(
              (res: any) => {
              ...
              (err) => {
                ...
                this._notify.show(
                  "",
                  "Hemos tenido un error al intentar mover el producto.",
                  "warning"
                );
                throw err;
              }
      ...
      this._notify.show(
        "",
        "No puedes cambiar el estado debido a que el producto no ha llegado al casillero.",
        "info"
      );
    }
  }
  ...
  disabledInputs(): void {
    if (
      this.shippingToUpdate.status != "0" &&
      this.shippingToUpdate.status != "1"
    ) {
  ...
  displayFnUserName(name: any) {
    if (name) {
      return name
        ? "CA " + name.locker[0].id + " | " + name.name + " " + name.last_name
        : "";
    }
  }
  ...
  generateLabel() {
    ...
    if (this.updateShippingForm.getRawValue()) {
      for (
        let index = 0;
        index < this.updateShippingForm.getRawValue().products.length;
        index++
      ) {
        ...
        if (product.arrived) {
          UID +=
            "-" +
            this.updateShippingForm.getRawValue().products[index].product.id;
        }
      }
      this._label
        .exportToLabel(
  ...
  goToFragment() {
    if (this.updateShippingForm.getRawValue().id) {
      this._router.navigate([
        "/fragment/" + this.updateShippingForm.getRawValue().id,
      ]);
    ...
    this._orderService
      ...
      .subscribe(
        (res: any) => {
          ...
          this._notify.show(
            "Orden de envío actualizada correctamente.",
            "",
            "success"
          );
        },
        (err) => {
          ...
          this._notify.show(
            "Error",
            "No pudimos actualizar la orden, intenta de nuevo.",
            "error"
          );
          throw err;
        }
  ...
  updateShipping(): void {
    if (this.status == 2) {
      if (
        this.updateShippingForm.controls.guide_number.value == "" ||
        this.updateShippingForm.controls.guide_number.value == null ||
        this.updateShippingForm.controls.conveyor.value == "" ||
        this.updateShippingForm.controls.conveyor.value == null
      ) {
    ...
    if (
      this.updateShippingForm.valid &&
      this.updateShippingForm.value.products.length > 0
    ) {
      this.updateConsolidate();
    } else {
      this._notify.show(
        "",
        "Revisa el formulario hay campos requeridos incompletos.",
        "info"
      );
    }
  }
